// docs/plans/worries-list.md "데이터/타입 설계" 기반 (이슈 #51: 일시정지 기능 제거로 관련
// 타입/필드 삭제 — CONTEXT.md "일시정지" 용어도 함께 삭제됨).
import type { WorryRecord } from '../lib/worries';

export type WorryListFilter = 'all' | 'ongoing' | 'pending';

/** status==='ongoing'인 WorryRecord에 로컬 오버레이를 얹어 계산하는 화면 표시용 파생 상태.
 *  'confirmed'는 로컬로 구매/포기 확정된 것 — 리스트에서 완전히 제외되므로 화면엔 노출되지 않지만
 *  파생 계산 중간값으로만 쓰인다. */
export type WorryDerivedStatus = 'ongoing' | 'pending' | 'confirmed';
...
export interface WorryListView {
  ...
  displayRemainingSeconds: number; // pending이면 0, 아니면 실시간 계산값
  displayProgressPercent: number;
  /**
   * ongoing 상태일 때 카드 내부 useCountdown이 실시간으로 갱신할 목표 시각(epoch ms,
   * worry.deadlineAt 그대로). pending은 카운트다운을 갱신하지 않으므로 null.
   */
