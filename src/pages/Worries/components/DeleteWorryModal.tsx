...
/**
 * docs/plans/worries-list.md 화면 3 — 진행 중 카드의 "✕"에서 뜨는 삭제 확인 모달(이슈 #51:
 * 일시정지 카드 variant는 제거됨). 기존 `ConfirmModal`(알림 삭제용, 260×100 텍스트 버튼 2개)과
 * 시각 스펙이 크게 달라 재사용하지 않고 전용 컴포넌트로 구현(계획서 확정 사항).
 *
 * "삭제"는 실제 DB delete가 아니라 확정과 동일하게 로컬 오버레이(deletedIds)로만 화면에서
 * 제외한다 — 새로고침하면 다시 나타난다(ADR-0004 확장 범위). 본문 카피는 원본 피그마의
